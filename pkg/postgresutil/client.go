...
func NewClient(addr, username, password string, timeout int) (IClient, error) {
	...
	dsn := fmt.Sprintf("host=%s port=%d user=%s password=%s sslmode=disable connect_timeout=%d", host, port, username, password, timeout)

	c.db, err = sql.Open("postgres", dsn)
	if err != nil {
		return c, fmt.Errorf("open PostgreSQL connection failed (dsn=%s): %w", dsn, err)
