...
// ClusterNode is the information of the targetted node
// Friends are the view of the other nodes from the targetted node
type NodeInfos struct {
	Node    *ClusterNode
	Friends ClusterNodes
}
...
func DecodeNodeStartTime(input *string, log logr.Logger) (time.Time, error) {
	...
	for _, line := range lines {
		...
		if values[0] == "uptime_in_seconds" {
			...
			if err != nil {
				log.Error(err, fmt.Sprintf("failed to parse Redis instance uptime from string: %s", values[1]))
				return time.Now(), err
			}
			return time.Now().Add(-time.Duration(uptimeInSeconds) * time.Second), nil
		}
	}
	log.Error(fmt.Errorf("failed to find uptime_in_seconds in Redis INFO output"), "")
	return time.Now(), fmt.Errorf("failed to find uptime_in_seconds in Redis INFO output")
}
	...
	for _, line := range lines {
		...
		if len(values) < 8 {
			...
			if ip, port, err := splitHostPort(ipPort[0]); err == nil {
				...
				log.Error(err, fmt.Sprintf("failed to parse IP:port from node info for node '%s': '%s'", node.ID, values[1]))
			}
			_ = node.SetRole(values[2])
...
func getConfigSignature(nodes ClusterNodes) ConfigSignature {
