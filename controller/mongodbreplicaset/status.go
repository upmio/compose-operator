...
func compareNodes(nodeA, nodeB *composev1alpha1.MongoDBReplicasetNode, reqLogger logr.Logger) bool {
	...
	if utils.CompareStringValue("Node.State", nodeA.State, nodeB.State, reqLogger) {
		reqLogger.Info(fmt.Sprintf("found status.Topology[Node].State changed: the old one is %s, new one is %s", nodeB.State, nodeA.State))
		return true
	}
...
func generateTopologyStatusByReplicationInfo(info *mongoutil.ReplicaSetInfos, instance *composev1alpha1.MongoDBReplicaset) {
	...
	for _, node := range instance.Spec.Member {
		...
		if nodeInfo, ok := info.Infos[addr]; ok {
			...
			instance.Status.Topology[node.Name].State = nodeInfo.State
		} else {
			allNodeReady = false
		}
	}

	if info.Status == mongoutil.ReplicaSetInfoConsistent && allNodeReady {
